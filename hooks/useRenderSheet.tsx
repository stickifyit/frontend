...
export default function RenderSheet({w}: Props) {
    ...
    useEffect(() => {
        ...
        sheet.forEach((item,i) => {
            for(let i=0;i<item.quantity;i++){
                flat.push({...item,quantity:1,size:item.size*w/(sheetW)-.8,fileType: item.fileType})
            }
        })
        setFinalSheet(flat)
            const Cm = w/sheetW
    ...
    } key={i} className={'absolute duration-200 rounded m-0 p-1'} style={{width:item.width +"px",height:item.height +"px",top:item.y ,left:item.x}}>
